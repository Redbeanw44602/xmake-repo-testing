package("td")
    ...
    on_install(function (package)
        ...
        if package:is_cross() then
            ...
            os.mkdir("auto/tlo")
            os.exec("tl-parser -e auto/tlo/mtproto_api.tlo scheme/mtproto_api.tl")
            os.exec("tl-parser -e auto/tlo/secret_api.tlo scheme/secret_api.tl")
            os.exec("tl-parser -e auto/tlo/e2e_api.tlo scheme/e2e_api.tl")
            os.exec("tl-parser -e auto/tlo/td_api.tlo scheme/td_api.tl")
            os.exec("tl-parser -e auto/tlo/telegram_api.tlo scheme/telegram_api.tl")
            os.cd("auto")
            os.mkdir("td")
            os.mkdir("td/telegram")
            os.mkdir("td/mtproto")
            print("Generate MTProto API source files")
            os.exec("generate_mtproto")
            print("Generate common TL source files")
            os.exec("generate_common")
            print("Generate JSON TL source files")
            os.exec("generate_json")
            print("Generate MIME Types source files")
            os.cd("../../../tdutils/generate")
            os.mkdir("auto")
            os.exec("generate_mime_types_gperf mime_types.txt auto/mime_type_to_extension.gperf auto/extension_to_mime_type.gperf")
